...
class CarState(CarStateBase):
    ...
    def update(self, can_parsers) -> structs.CarState: # type: ignore
        ...
        # BYD specific: Handle standstill logic for proper longitudinal control
        # When cruise is enabled and we're physically stopped, we still report standstill as False
        # to allow the longitudinal planner to enter 'starting' state when needed
        # The actual standstill state is preserved in ret.standstill for other uses
        if ret.cruiseState.enabled and ret.standstill:
            # Allow starting when cruise is active - this helps with BYD's starting logic
        ...
        # BYD specific: Set softHoldActive based on standstill and cruise state
        # This helps CarrotPlanner's XState machine work properly for BYD vehicles
        if ret.cruiseState.enabled and ret.standstill and ret.vEgo < 0.1:
            # When cruise is active and vehicle is stopped, activate soft hold
            # This allows CarrotPlanner to enter e2eStopped state for proper restart logic
            ret.softHoldActive = 2  # 2 = activated (as per car.capnp definition)
        elif ret.cruiseState.enabled and ret.standstill:
            ret.softHoldActive = 1  # 1 = active ready
        else:
            ret.softHoldActive = 0  # 0 = not active
